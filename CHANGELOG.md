...
*   **Qué Queda de los 5 Registros**: **286** y **295** recuperan su horario real en su próximo login o al abrir su ficha; **315** y **290** pasan a `NULL` («No disponible»), que es lo correcto porque RH no tiene turno para ellos. **4** (Centro de Soporte Humanitas) no tiene `clave` y no inicia sesión, así que solo se limpia con una escritura directa —pendiente, y sin efecto práctico: con `NULL` o con los días vacíos queda igualmente fuera del reparto automático.
