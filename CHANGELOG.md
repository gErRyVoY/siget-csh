...
*   **La Regla Real, Explicitada**: las asignaciones de categorías (`asignaciones_categorias`) gobiernan **solo el reparto automático** — dicen a quién le puede caer un ticket sin que nadie lo pida. La **asignación manual** es deliberadamente más ancha: cualquier ingeniero de la cola puede tomar un ticket sin asignar, y un superadmin puede reasignarlo a quien haga falta.
*   **El Bug**: `cargarAgentesAsignables()` (desplegable) y `canAgentBeAssignedManually()` (validación del PATCH) implementaban criterios **distintos**. El desplegable pedía «activo **y** (flag `atiende_*` **o** categoría asignada)»; el validador exigía además una fila **exacta** de `asignaciones_categorias` para esa categoría/subcategoría, con caída a `permiso_categoria` (tabla que en producción tiene sus 92 filas en `activo = false`). Medido sobre los 21 tickets abiertos: de **125** candidatos ofrecidos, **41 (33 %)** los rechazaba el guardado con un 400.
*   **El Caso Bloqueado**: el ticket **#33** (cat. 2 «Aspirante», subcat. 143 «Cambio de ciclo») ofrecía 8 candidatos y el validador rechazaba **los 8**, incluido el agente que ya lo atendía. Cualquier guardado de un usuario privilegiado sobre ese ticket —incluso añadir solo un comentario— devolvía `400 El usuario no tiene habilitada esta categoría/subcategoría`.
*   **La Corrección**: el criterio vive en una única función, `buildManuallyAssignableWhere()` de `src/services/ticketAssignmentService.ts`, que consumen el desplegable y el validador. `canAgentBeAssignedManually()` ya no consulta `asignaciones_categorias` ni `permiso_categoria`: aplica ese `where` con el `id` del agente y, si no pasa, distingue «no está activo» de «no atiende esta cola» para que el 400 diga algo accionable.
*   **Medición Posterior**: mismos 21 tickets, mismos 125 candidatos ofrecidos, **0 rechazos**. El reparto automático no cambia: `findBestAgentHybrid`, `resolveSpecificAgents` y `findAgentsByRolePermissions` siguen exigiendo la categoría, y Marketing sigue entrando «Sin asignar».

### Fix: El Horario Roto se Repara al Iniciar Sesión (y se Limpia si RH no Tiene Ninguno)

*   **Por Qué no se Reparaba Solo**: la guarda estaba escrita como `if (!user.horario_disponibilidad)`, y un objeto con los seis días a `null` —`{"lunes":{"inicio":null,"fin":null}, …}`— es **truthy**. Esas filas se daban por buenas y no se volvía a consultar a RH nunca. Además el login solo escribía la columna cuando la consulta devolvía un horario utilizable, así que un valor inservible sobrevivía a cada inicio de sesión.
*   **Validación de la API de RH** (`GET /api/rh/horario-trabajador`, consultada con las claves reales): responde correctamente. `FTJ1566` (Jair Flores) y `TLA1268` (Ariadna Tinoco) devuelven `200` con sus seis y cinco días; `MPP1562` (Paulina Mondragón) y `PXJ123` (Juan Pérez, usuario de prueba) devuelven `404 {"detail":"No se encontró un horario asignado para este trabajador."}`. Hay colaboradores sin turno en la nómina y es un caso normal, no un error.
*   **Un Solo Cliente, con Tres Estados**: nuevo `src/lib/rh-horario.ts` (`consultarHorarioRH`, `parseDiasLaborales`, `tieneHorarioUtil`, `horarioParaPrisma`), que sustituye las dos copias divergentes del login y de la ficha. La consulta devuelve `ok` (se escribe el horario), `sin-horario` (**se escribe `NULL`**, que es lo que limpia las filas dañadas) o `indeterminado` —red, 5xx, forma inesperada— en cuyo caso la columna **no se toca**.
*   **Dónde se Aplica**: `auth.config.ts` en cada login y `/admin/usuarios/editar/<id>` al abrir la ficha, ambos con la guarda `tieneHorarioUtil()` en lugar de la comprobación de verdad/falsedad. Un fallo de red al pedir el horario ya no bloquea el login (antes caía en el `catch` que devuelve `error=ColaboradorNoActivo`); la consulta que sí lo bloquea sigue siendo `consultar-trabajador`.
*   **`null` en una Columna `Json?`**: Prisma no acepta un `null` a secas ahí, exige `Prisma.DbNull`. `/user/perfil` y `/api/admin/usuarios` mandaban `null` cuando no quedaba ningún día utilizable, así que ese guardado habría respondido 500; los tres puntos de escritura pasan ahora por `horarioParaPrisma()`.
*   **La Clave del Trabajador se Recorta**: la API la devuelve rellenada a diez caracteres (`"FTJ1566   "`) y se guardaba tal cual en usuarios nuevos.
*   **Qué Queda de los 5 Registros**: **286** y **295** recuperan su horario real en su próximo login o al abrir su ficha; **315** pasa a `NULL` («No disponible»), que es lo correcto porque RH no tiene turno para él. **290** y **4** se pusieron a `NULL` a mano (ver abajo).

### Fix: `soporte@humanitas.edu.mx` no Podía Iniciar Sesión, y las Cuentas de Prueba se Declaran en un Solo Sitio

*   **El Bug**: `signIn` decidía «esto es un usuario de prueba» con un literal, `profile.email === 'alumno.prueba1@humanitas.edu.mx'`. Cualquier otra cuenta de servicio pasaba por Google Admin Directory, por la validación de OU y por la API de RH. Y RH **no conoce** `soporte@humanitas.edu.mx`: `consultar-trabajador` responde `404 {"detail":"No se encontró registro para el correo: …"}`, que `signIn` traduce a `/login?error=ColaboradorNoActivo`. El buzón del Centro de Soporte (usuario **4**) no podía entrar, ni con la `clave` `CSH1979` que se le asignó en SiGeT — `horario-trabajador` también devuelve 404 para ella.
*   **La Corrección**: nuevo `src/config/test-users.ts` con `TEST_USER_EMAILS` e `isTestUserEmail()`, que ahora incluye `alumno.prueba1@` y `soporte@`. Para esos correos se omiten las tres comprobaciones externas: Directory, OU y las dos consultas de RH. La ficha de `/admin/usuarios/editar/<id>` aplica la misma exclusión, para no gastar una petición a RH en cada carga sabiendo que va a fallar.
*   **Contrapartida Documentada**: su fila debe existir ya en la base de datos, porque el alta automática de `signIn` deriva el campus de la OU. Y no tendrán horario: RH no les asigna turno, así que `horario_disponibilidad` se queda en `NULL`.
*   **Limpieza de Datos Aplicada**: `horario_disponibilidad = NULL` en los usuarios **4** y **290**. El 290 ya estaba limpio (lo hizo el código nuevo al abrir su ficha), así que el `UPDATE` tocó **1 fila**. Barrido posterior: de 44 usuarios con horario no nulo quedan **3 inservibles** (286, 295 y 315), que son colaboradores reales y los repara el propio código en su próximo login o al abrir su ficha.

## 2026-09-12 (El `<select>` de Rol Reconfigura la Ficha del Usuario al Instante)

### UX: `/admin/usuarios/editar/<id>` Muestra las Opciones del Rol Seleccionado sin Guardar

*   **Lo Que Obligaba a Hacer**: la visibilidad de cada bloque configurable se decidía **solo en el servidor**, contra el rol **guardado** (`targetRoleId = user.rolId`). Convertir a un Usuario en Admin exigía tres pasos: cambiar el rol, guardar, esperar la recarga y entonces configurar «Asignar tickets», «Atiende CSH/Mkt» y los auditores, que hasta ese momento no existían en el DOM.
*   **Unión Renderizada + Metadatos por Rol**: la vista pinta ahora **todos** los bloques y cada uno declara en `data-roles` los roles que lo habilitan (`[2,3]` para Asignar/Atiende/Auditor, `[1,2]` para Levanta CSH/Mkt, `[2]` para Horario, `[1,2]` para Secciones, `[2,3]` para Categorías). El servidor sigue fijando el estado inicial en `data-rol-hidden` para que no haya salto al cargar, y `applyRoleVisibility()` lo recalcula al cambiar el `<select>`.
*   **Por Qué `data-rol-hidden` y no la Clase `hidden`**: los bloques que se ocultan traen sus propias utilidades de `display` (`flex`, `grid`), que empatarían en especificidad con una clase de Tailwind y dependerían del orden del CSS generado. La regla `[data-rol-hidden="true"] { display: none !important }` no admite ambigüedad.
*   **Lo Que Decide el Guardado Pasa de «Existe» a «Existe y se Ve»**: `performSave()` omitía un flag booleano cuando su `<input>` no estaba en el DOM, precisamente para no sobrescribir la BD con `false`. Al renderizar la unión, ese elemento existe **siempre**, así que la comprobación habría empezado a mandar flags que antes se omitían. Ahora la condición es la visibilidad (`el.closest('[data-rol-hidden="true"]') === null`), y el comportamiento observable es idéntico al de antes.
*   **Los Filtros del Editor se Quedan en el Servidor**: `isTargetSelfSuperAdmin`, `areAuditorChecksHidden` (equipo de Marketing) y `canEdit` describen a **quién edita**, no al rol objetivo, así que mover el `<select>` no los cambia y siguen resolviéndose en el frontmatter.
*   **Las Vistas «Todos» ya no se Filtran en el Servidor**: `soporte_todos` y `marketing_todos` se pintaban solo si el rol guardado no era Usuario. Ahora llevan su propio `data-roles` y aparecen y desaparecen con el `<select>`; su subgrupo se oculta únicamente si **todas** sus secciones son de ese tipo.
*   **Secciones y Categorías se Bloquean con un Cambio de Rol Pendiente**: esos toggles guardan al instante contra sus propios endpoints, pero el `$transaction` de `/api/admin/usuarios` **purga `permisoUsuarioSeccion` y `asignacionesCategorias`** cuando el `rolId` cambia. Configurarlos antes de guardar habría sido trabajo perdido sin aviso, así que mientras el `<select>` no coincida con el rol guardado quedan deshabilitados y se muestra el motivo. `setSectionChecked()` y la sincronización de `cat-12` también se cortan, para no disparar PATCH condenados a desaparecer. `rolLockBase` conserva el `disabled` que puso el servidor, de modo que desbloquear no habilita de más.

### Fix: Guardar la Ficha de un Usuario ya no Destruye su Horario

*   **El Bug**: los `<select>` deshabilitados no viajan en el `FormData`, así que `formData.get('lunes-inicio')` devolvía `null`; la condición `inicio !== 'No disponible'` daba `true` para `null` y **cada guardado** escribía `horario_disponibilidad = {lunes:{inicio:null,fin:null}, …}` con los seis días. El horario de esta vista es de solo consulta (lo sincroniza la API de RH al iniciar sesión), de modo que se destruía sin que nadie tocara nada.
*   **Consecuencia Medida**: `filterBySchedule()` exige `inicio` y `fin` para considerar disponible a un agente, así que esos usuarios quedaban fuera de la asignación automática de forma permanente. En producción lo tienen **5 usuarios**, los 6 días a `null`: **Paulina Mondragón** (315), **Centro de Soporte Humanitas** (4), **Jair Flores Téllez** (286), **Ariadna Tinoco** (295) y **Juan Pérez** (290) — cuatro de ellos con rol Admin y activos. Jair Flores Téllez ya figuraba en la lista de «horarios inválidos» pendientes de revisar; esta es la causa.
*   **La Corrección**: `horario_disponibilidad` solo se incluye en el PATCH si el contenedor existe, **no** es de solo lectura y está visible para el rol seleccionado; y dentro se descartan los días cuyo valor no sea una cadena. Con la vista tal y como está hoy eso significa que nunca se envía, que es exactamente lo que debía pasar.
*   **Los 5 Registros Dañados no se Han Tocado**: son un cambio de datos en producción, no de código.

### Fix: El Formulario ya no se Marca como «Sucio» de Más

*   `initialSnapshot` se calculaba **antes** de que `initHorarios()` insertara los `<select>` del horario, así que el primer cambio en cualquier campo se comparaba contra un snapshot sin esas claves y el aviso de «cambios pendientes» salía siempre. El snapshot se recalcula tras `initHorarios()`.
...
### UX: El Logo del Menú Lateral Lleva al Dashboard que de Verdad Usa la Persona (`src/components/shared/Sidebar.astro`)
*   **Comportamiento Anterior**: El logo apuntaba a `/` con un `href` fijo, es decir al dashboard de **soporte**, para todo el mundo. Para un admin que solo atiende Marketing eso significaba que el gesto más natural del menú —pulsar el logo para volver al inicio— lo sacaba de su circuito.
*   **Regla**: el destino pasa a `/tickets/marketing/dashboard` cuando **todas** las asignaciones activas del usuario en `asignaciones_categorias` son de Marketing (y hay al menos una). Una asignación cuenta como de Marketing por su `categoriaId` o, si la fila no fija categoría, por la categoría de la que cuelga su `subcategoriaId` a través de `subcategoria_categorias` — la petición hablaba de "la categoría y/o una subcategoría".
*   **Por Qué `asignaciones_categorias` y no `permiso_categoria`**: la primera es el mecanismo vivo (2 055 filas); la segunda tiene sus 92 filas con `activo = false` y nunca entra en juego (ver la auditoría más abajo).
*   **El Destino Tiene que Ser Alcanzable**: `/tickets/marketing/dashboard` está protegido por la sección `marketing_dashboard`, así que la comprobación solo corre para quien la tiene. Sin ese cuidado, el logo habría mandado al usuario a una ruta que el middleware devuelve a `/` con el aviso de acceso denegado.
*   **A Quién Afecta, Comprobado en Producción**: de los 14 admins/superadmins activos, exactamente **cinco** cumplen la condición — **Haide Herrera** (id 2, 18 asignaciones activas, todas de Marketing), **Mariam Monroy** (293), **David Ocampo** (294), **Ariadna Tinoco** (295) y **Karla Gonzalez** (296). El resto, y cualquier usuario de rol 1, siguen yendo a `/`. Ninguna subcategoría está compartida entre Marketing y otra categoría, así que no hay casos ambiguos.
*   **Coste en Consultas**: las dos consultas nuevas van en el mismo `Promise.all` que el `findFirst` de `hasMarketingTickets` que ya existía, así que el Sidebar sigue costando **un viaje** a RDS por render, y para quien no tiene `marketing_dashboard` ni se emiten.

### Feature: Los Tickets de Marketing Entran «Sin Asignar» y los Reparte el Equipo

*   **Motivo**: Marketing no funciona como soporte. Una pieza de diseño no es intercambiable entre personas por carga de trabajo: el equipo mira lo que entra y decide quién lo toma. La asignación automática elegía por ellos.
*   **Sin Asignación Automática (`src/services/ticketAssignmentService.ts`)**: `findBestAgentHybrid()` corta en un **PASO 0** cuando la categoría es Marketing y devuelve `{ agentId: null, assignmentType: 'none' }`. Se eligió ese punto porque es el **único** sitio donde se decide el agente en el alta: `create.ts:174` ya tiene la rama de "sin agente" que crea el ticket con `estatusId: 1` («Sin asignar») y manda el correo `ticket_sin_asignar` al solicitante, así que no hubo que tocar nada más. El otro consumidor del servicio, `transfer.ts:142`, es siempre de soporte.
*   **Lo Que Hacía Antes**: el PASO 2 asignaba **a la fuerza** en cuanto quedaba un único candidato, ignorando horario y `acepta_tickets`. Con cinco personas asignadas a Marketing hoy no se disparaba, pero bastaba con que cuatro estuvieran de baja para que el quinto recibiera todo automáticamente y fuera de horario.
*   **El Triaje Necesitaba Permiso (`src/pages/tickets/view/[id].astro` y `src/pages/api/tickets/update.ts`)**: la regla de edición del campo «Atiende» era «superadmin siempre; admin **solo si el ticket ya está asignado a él mismo**». Un ticket sin dueño no puede cumplirla nunca, así que dejar los tickets sin asignar habría creado un callejón sin salida: solo un superadmin podría dar el primer paso. Se añade una tercera vía — mientras el ticket de Marketing siga con `atiendeId` nulo, cualquier admin del equipo de Marketing puede asignárselo o asignarlo a otra persona del equipo.
*   **Quién Es "del Equipo de Marketing"**: la nueva `canAssignMarketingTickets()` exige las dos condiciones que se administran en `/admin/usuarios/editar/<id>`: asignación **activa** a Marketing en `asignaciones_categorias` (por la categoría o por alguna de sus subcategorías) **y** «Atiende Mkt» (`atiende_mkt`), propio o heredado del rol. La segunda no es decorativa: sin ella la persona ni siquiera puede *recibir* un ticket de Marketing (`hasCorrectUserFlag`), así que tampoco tiene sentido que reparta los de los demás.
*   **La Regla Vive en Dos Sitios a Propósito**: la vista la usa para no renderizar el `<select>` como `disabled`, y el endpoint para autorizar el PATCH. Quitar el `disabled` del DOM sin la mitad del servidor no habría servido de nada — y al revés, el servidor sin la vista habría dejado el desplegable gris.
*   **Comprobado Contra Producción**: los 5 tickets de Marketing existentes (#23, #41, #43, #46, #47) están todos sin asignar. Antes solo los superadmins (ids 1 y 3) podían asignarlos; ahora también los **cinco** admins con Marketing asignado y `atiende_mkt`: **Haide Herrera** (2), **Mariam Monroy** (293), **David Ocampo** (294), **Ariadna Tinoco** (295) y **Karla Gonzalez** (296). Los 35 tickets que no son de Marketing no cambian de permisos.
*   **Se Incluye al Solicitante**: la vía nueva no excluye a quien levantó el ticket. Excluirlo reproduciría el mismo bloqueo cuando quien lo abre es del propio equipo, que es el caso habitual en Marketing.
*   **La Ventana se Cierra al Asignar**: en cuanto el ticket tiene dueño pasa a «Nuevo» (transición automática de `update.ts:233`) y vuelve a regir la regla de siempre: solo lo reasigna la persona asignada o un superadmin.
*   **Efecto Colateral Corregido**: esa transición automática a «Nuevo» no comprobaba `canEditAtiende`, así que un PATCH que trajera `atiendeId` de alguien sin permiso movía el estatus **sin** asignar a nadie. Es exactamente el estado en el que está el ticket **#23** (estatus «Nuevo» con `atiendeId` nulo). Con los tickets de Marketing entrando siempre «Sin asignar» ese desajuste habría sido fácil de provocar, así que la transición pasa a exigir el permiso.

### Refactor: `MARKETING_CATEGORY_ID` Centralizado en `src/config/ticket-categories.ts`
*   **Problema**: El literal `12` estaba repetido en **doce** archivos (servicio de asignación, las cuatro listas, los dos dashboards, el detalle de ticket, las dos vistas de alta —esta última con su propio `const catId = 12`— y el Sidebar, con un `categoriaId: 12` a pelo). Un cambio de id habría dejado media aplicación mirando a la categoría equivocada. Corresponde al hallazgo de la línea 117 de `MEJORAS-AUDITORIA-2026-08-25.md`.
*   **Módulo Nuevo**: exporta `MARKETING_CATEGORY_ID`, los identificadores de sección `MARKETING_CREATE_SECTION` / `CSH_CREATE_SECTION` y el helper `isMarketingCategory()`. Queda **una declaración y doce importadores**.
