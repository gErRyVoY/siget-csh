...
- ✅ Reseteo automático de excepciones (`PermisoUsuarioSeccion`/`AsignacionesCategorias`) al cambiar el rol de un usuario.
- ✅ Nueva vista administrativa global (`/admin/roles`) para modificar configuraciones maestras de Roles y Clonar roles.
...
*   **Gestión Global de Roles (`/admin/roles/index.astro`):**
    *   **Inactivación de Roles:** En lugar de eliminar un rol (lo que rompería la integridad referencial), se implementó un Toggle Switch global en el encabezado de configuración de cada rol. Esto permite activar/inactivar, y el select principal le añade la leyenda "`(inactivo)`" a las opciones apagadas.
    *   **Dashboard de Asignados:** Debajo de la configuración de un rol, se renderiza iterativamente una sección expansiva (`<details>`) listando a todos los empleados atados a dicho perfil, agrupados elegantemente bajo el título de su respectiva *Empresa*.
    *   **Expulsión a Visitante (Revocación):** Cada empleado del listado incluye un Toggle propio. Apagarlo despierta el endpoint especial `/api/admin/roles/revoke-user` que, además de reasignarle el id del rol primigenio "Visitante", aplica una expurgación (`deleteMany`) de todas sus sentencias de `PermisoUsuarioSeccion`/`AsignacionesCategorias` atadas a su perfil (evitando permisos residuales rotos).
*   **UI/UX Perfil (`/admin/usuarios/editar/[id].astro`):** Se adaptó el dropdown (Select) que cambia el rol de los usuarios individuales, encubriendo preventivamente cualquier rol cuyo estatus se halle en inactivo (omitiéndolo de la lista para prevenir asignaciones).

## 2026-03-30 (Sesión 19 - Gestión Global de Roles y Reseteos)
*   **Perfil de Usuario (`editar/[id].astro` y API):**
    *   **Reseteo Automático:** La API `/api/admin/usuarios.ts` ahora detecta si a un usuario se le cambió exitosamente de rol y procede a eliminar de la BD todas sus excepciones/permisos específicos (tando de Secciones como de Categorías), dejando un "lienzo en blanco" puro para el nuevo rol asignado.
    *   **Recargo Inteligente:** Tras guardar cambios en el formulario que impliquen un nuevo Rol, Astro despacha una alerta visual al administrador y fuerza una recarga total de pantalla (`window.location.reload()`) tras 1.5s para forzar el recálculo SSR (Server Side Rendering) de los interruptores y Toggles.
*   **UI/UX Global de Roles (`/admin/roles/index.astro`):**
    ...
    *   **Acceso en Sidebar:** Añadida la sección "Roles" en el menú de navegación del administrador.
    *   **Formulario Dinámico:** Un selector SSR superior que carga dinámicamente (`?id=X`) la configuración global de un rol.
    *   **Clonación:** Se integró la función "Nuevo Rol" mediante un menú ocultable para re-usar las tablas generadas clonando directamente registros de `PermisoCategoria` y `PermisoRolSeccion` en un sólo paso transaccional (API: `/api/admin/roles/create.ts`).

## 2026-03-19 (Sesión 18 - Refactorización Perfil Usuario y UI)
*   **Edición de Usuarios (`[id].astro`):**
    *   **Layout:** Se reordenaron los campos `Empresa` y `Rol` hacia la parte superior.
    *   **Toggles:** Se reemplazaron los checkboxes booleanos (Activo, Vacaciones, Auditores) por Switches Toggle en un grid 2x2.
    *   **Acordeones Uniformes:** Se implementaron secciones colapsables (`<details>`) con altura de 50px para Horarios, Secciones y Categorías, incorporando comportamiento "single-open" auto-excluyente y estilos interactivos consistentes (fondo vino).
    *   **Permisos de Categorías:** Se creó una nueva matriz de asignación jerárquica para permitir/revocar el acceso del usuario a categorías/subcategorías de Tickets mediante checkboxes integrados al menú.
*   **Backend & APIs:**
    *   Nuevo endpoint `/api/admin/usuarios/categorias` para reaccionar optimista y velozmente a los cambios de permisos en la tabla `AsignacionesCategorias`.
*   **Correcciones Transversales:**
    *   Se resolvió un warning de tipos TypeScript (string | undefined) en `/admin/secciones`.
    *   Se automatizó la vista de `/admin/ciclos` invocando el bloque en servidor `ensureActiveCycle()` antes de renderizar, garantizando congruencia visual con la fecha actual del sistema.

## 2026-03-19 (Sesión 18 - Adjuntos en Nuevo Ticket CSH)
*   **Interfaz de Creación (`nuevo-ticket-csh.astro`):**
    *   **UI/UX:** Se añadió la zona de "Arrastrar y soltar" (Drop Zone) para adjuntos, junto con los botones de subida local e integración con Google Drive, replicando la experiencia nativa de la vista de Traslados.
    *   **Integración:** Se inyectaron credenciales seguras de Google Cloud (`API_KEY`, `CLIENT_ID`, `APP_ID`) vía atributos `data-*` al formulario para montar Google Picker API sin exponer tokens blandos pre-procesados.
*   **Controlador Avanzado (`ticket-wizard.ts`):**
    *   **Google Picker:** Se adaptó la lógica paralela de selección en la nube (Google Drive), descargando los buffers en memoria local antes de subirlos a los servidores AWS.
    *   **Subida a AWS S3:** Se modificó el núcleo del envío `handleSubmit` para gestionar primero la creación del ticket (`/api/tickets/create`), luego invocar concurrentemente `Promise.all` para subidas S3 firmadas URL (`/api/tickets/generate-upload-url`), y finalmente asociar todas las referencias generadas al ticket por medio de una petición con método `PATCH`.
    *   **Gestor Local:** Validación total de colas de subida interactivas (array límite de 10 files, tamaño restringido a 5MB por file).

## 2026-03-19 (Sesión 18 - Refinamiento de Vista de Ticket y Notificaciones)
*   **Notificaciones SSE:**
    *   Se actualizó `update.ts` para enviar el último comentario dentro del *payload* SSE cuando se edita un ticket.
    *   Se actualizó `list.ts` para los usuarios con privilegios (Resolvers) adjuntando el último historial `comentario` en la carga inicial de la campana.
    *   Se modificó `MainLayout.astro` para renderizar el último comentario como subtítulo de la notificación; si no hay comentario nuevo, hace fallback a la descripción original del ticket.
*   **UI de Vista de Ticket (`[id].astro`):**
    *   Se reemplazó el checkbox nativo de "Archivado" por un **Toggle Switch** interactivo consistente con el resto del sistema.
    *   Se reestructuró la zona de adjuntos (Drop Zone) dividiéndola en un layout de 2 columnas (`grid-cols-2`). Del lado izquierdo se encuentra el área para arrastrar, y del lado derecho se organizó la directiva (leyenda) y los botones de acción (`Adjuntar` / `Google Drive`).
