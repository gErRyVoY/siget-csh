...
**Tarea Actual:** Completada — Consulta a API de RH en Inicio de Sesión y Complementación de Clave ✅

**Estado:** Completado. Se actualizó la URL por defecto de la API de Recursos Humanos en `auth.config.ts` a `https://pz3bmmqsty.us-east-1.awsapprunner.com` (producción). También se modificó la rama de actualización del usuario durante el login de tal manera que el campo `clave` se complemente únicamente si está vacío en la base de datos, previniendo la sobreescritura de claves ya asignadas.

**Pasos Siguientes:**
1. Monitoreo y validación de las actualizaciones de clave de usuario al iniciar sesión.
