...
**Tarea Actual:** Completada — Reestructuración de Roles y Permisos (Esquema de 3 Roles) ✅

**Estado:** Completado. Se reestructuró la base de datos para migrar a un nuevo modelo plano de 3 roles (`user`, `admin`, `superadmin`). Se modificaron las tablas de `Rol`, `Usuario` y `Empresa` para admitir flags booleanos explícitos de permisos individuales. Se actualizó la lógica de inicio de sesión (`auth.config.ts`), el servicio de asignación de tickets (`ticketAssignmentService.ts`), las interfaces de administración de usuarios y campus (`editar/[id].astro` y `[campus].astro`), las APIs y las páginas de listado/detalle. Todo compila exitosamente.
