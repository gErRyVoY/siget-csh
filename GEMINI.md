...
**Pasos Siguientes:**
1. **Merge a `siget-apprunner-new` y despliegue** de los cambios del 2026-09-11 (ocultado de Marketing en el alta de CSH, autorización de `/api/tickets/create`, `flagRouteMap` del middleware y centralización de `MARKETING_CATEGORY_ID`). QA en producción después: que Haide Herrera (`tckt_mkt = false`) ya no vea la tarjeta del dashboard de marketing, y que quien sí tiene el flag pueda seguir abriendo tickets de marketing con normalidad.
...
*   **Configuración (`src/config/ticket-categories.ts`):**
    *   **`MARKETING_CATEGORY_ID` Centralizado:** El literal `12` estaba en once archivos. El módulo nuevo exporta además `MARKETING_CREATE_SECTION`, `CSH_CREATE_SECTION` e `isMarketingCategory()`.
