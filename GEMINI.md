...
**Tarea Actual:** Separación de los circuitos de Marketing y Soporte terminada en `nuevos-cambios-claude` (2026-09-11). Pendiente de merge a `siget-apprunner-new`, que es lo que despliega.

**Pasos Siguientes:**
1. **Merge a `siget-apprunner-new` y despliegue** de los cambios del 2026-09-11 (ocultado de Marketing en el alta de CSH, autorización de `/api/tickets/create`, `flagRouteMap` del middleware, destino del logo del Sidebar y centralización de `MARKETING_CATEGORY_ID`). QA en producción después: que Haide Herrera (`tckt_mkt = false`) ya no vea la tarjeta del dashboard de marketing y que el logo la lleve a `/tickets/marketing/dashboard`, que a cualquier otro admin el logo lo siga llevando a `/`, y que quien sí tiene el flag pueda seguir abriendo tickets de marketing con normalidad.
2. Decidir qué hacer con los hallazgos de la **auditoría de tablas y columnas sin uso** (inventario completo en `CHANGELOG.md`, entrada del 2026-09-11). Lo que no necesita migración se puede hacer ya: borrar `canViewAllTickets` y el `permisos: true` de la consulta de sesión, borrar `lifecycle.astro` y `test-sonner.astro`, cablear o eliminar la sección `plataforma_humanitas`, y quitar el `include: { bloque: true }` de `view/[id].astro`. Lo que toca esquema (las cuatro `rol.*`, la tabla `bloque`, `permiso`/`_PermisoToRol`, etc.) es **una migración de producción**: `prisma db execute` + `migrate resolve --applied`, nunca `migrate dev`.
3. Confirmar si el `activo = false` de las 92 filas de `permiso_categoria` es deliberado. Si lo es, la tabla y el fallback por rol de `ticketAssignmentService.ts` sobran y conviene retirarlos para que nadie asuma que existe una capa de permisos por rol que en realidad no corre.
4. Verificar las **dimensiones del diálogo de Google Picker** con la página desplazada (paso de adjuntos del asistente de alta). Requiere OAuth contra el Drive del usuario, el asistente no puede probarlo.
5. Confirmar con Rogelio Elizalde López su horario de viernes en `/user/perfil` (se presume `10:00–18:00`) para cerrar el diagnóstico del día 28 de agosto al 100%.
6. Corregir en el perfil los tres horarios inválidos detectados: Haide Herrera (sin `clave` y horario nulo), Jair Flores Téllez (seis días con `inicio`/`fin` en `null`) y Victor Barrera (sin sábado configurado).
7. Rotar las dos API keys de Google que se pegaron en una conversación con el asistente.
8. Deduplicar las filas repetidas de `asignaciones_categorias` (2 055 filas hoy); el código las tolera desde `f9437f4`, pero conviene hacerlo al crear la base nueva.

**Pasos Completados:**
- ✅ **Separación de los Circuitos de Marketing y Soporte (2026-09-11):** Marketing ya no aparece en el selector de `/tickets/soporte/nuevo-ticket-csh` — se filtra en la consulta y no en el cliente, porque el árbol de categorías se serializa completo dentro del HTML. `/api/tickets/create` pasó de comprobar solo que hubiera sesión a **autorizar por categoría**: Marketing exige `tckt_mkt` + sección `crear_ticket_marketing`, el resto `tckt_csh` + `crear_ticket_csh`, y si no se cumple responde **403**; los dos asistentes comparten endpoint, así que rechazar `categoriaId: 12` a secas habría roto el alta de marketing. El middleware ganó un `flagRouteMap` que exige el flag además de la sección en las dos vistas de alta: antes las tres roles tenían `crear_ticket_marketing`, así que los **90 usuarios con `tckt_mkt = false`** entraban escribiendo la URL. Se condicionó también la tarjeta "Levantar ticket" de `/tickets/marketing/dashboard`, que era una segunda entrada visible para **Haide Herrera** (id 2, admin, sin el flag). El logo del Sidebar dejó de apuntar siempre a `/`: quien tiene **todas** sus asignaciones activas de `asignaciones_categorias` en Marketing —Haide Herrera y `alumno.prueba1`, los dos únicos casos en producción— va al dashboard de marketing, y solo si tiene la sección `marketing_dashboard`, porque el middleware protege esa ruta. Y el literal `MARKETING_CATEGORY_ID = 12`, repetido en **doce** archivos, quedó en `src/config/ticket-categories.ts` junto con los identificadores de sección y `isMarketingCategory()`. Verificado contra datos reales que ningún alta histórica habría sido rechazada. `pnpm astro check` 0 errores / 0 warnings en 161 archivos y `pnpm build` completo.
- ✅ **QA en Producción del Plan de Rendimiento (2026-09-03):** Los cuatro puntos de más riesgo pasan. **1** — los filtros de `/tickets/soporte` funcionan y la fecha quedó corregida. **5** — el ticket de traslado carga todos los datos (campus, carrera, descuento y los dos auditores). **8** — quitar y devolver una sección a un rol en `/admin/secciones` se refleja de inmediato, sin esperar el TTL. **10** — la notificación llega casi al instante. Con esto se cierra la sección B del `RUNBOOK-CIERRE-PERF.md`.
- ✅ **Auditoría de Tablas y Columnas sin Uso (2026-09-11):** Barrido completo tras la actualización de roles, con conteo por columna vía `information_schema` y búsqueda de lectores y escritores en todo el proyecto. Inventario íntegro en la entrada del 2026-09-11 del `CHANGELOG.md`. Titulares: `bloque` y `permiso`/`_PermisoToRol` están muertas; `logs` y `notificaciones_correo` son write-only; las cuatro `rol.*` (`ticket_csh`, `ticket_mkt`, `traslados`, `generales`) son residuo de la actualización de roles y no las lee nadie; `permiso_categoria` está cableada pero 100 % inerte (`activo = false` en sus 92 filas contra tres consultas `activo: true`); y la sección `plataforma_humanitas` está activa sin gobernar nada. **No se aplicó ningún cambio de esquema.**
...
**Pasos Completados:**
- ✅ **Eliminación de las Vistas `/admin/roles` y `/admin/tickets` (2026-08-26):** Borradas ambas páginas y los 6 endpoints de `src/pages/api/admin/roles/` (`create`, `categorias`, `secciones`, `toggle-active`, `toggle-flags`, `revoke-user`), que solo consumía `/admin/roles`. Limpiadas las entradas de `sectionRouteMap` en `src/middleware.ts`, los 3 enlaces del `Sidebar.astro` (incluido el duplicado del grupo Admin Marketing), las secciones `admin_siget_roles` (id 21) y `admin_siget_tickets` (id 19) de `prisma/seed.ts` y las referencias de `scripts/validate-flows.ts` y `scripts/perf-baseline.ts`. Se sustituirán por un módulo nuevo. `npx astro check` 0 errores en 154 archivos + `pnpm build` OK.
- ✅ **Permisos Granulares de Edición en Detalle de Traslados, Autocompletado de Alumno por Matrícula y Corrección de Historial Fantasma (2026-08-25):** Atiende deshabilitado para admins no asignados; campus origen dinámico según `tckt_virtual`; alumno, carrera y escolarizada deshabilitados; matrícula reactiva con consulta a la API de alumnos; normalización de bloque y respuesta limpia con `hasNewHistoryEntry`. `npx astro check` 0 errores en 157 archivos.
...
## 2026-09-11 (Separación de los Circuitos de Marketing y Soporte)
*   **Alta de Soporte (`src/pages/tickets/soporte/nuevo-ticket-csh.astro`):**
    *   **Marketing Fuera del Selector:** `where: { activo: true, id: { not: MARKETING_CATEGORY_ID } }`. Se filtra en la consulta porque el árbol de categorías se serializa completo en el HTML (`<script id="categories-data">`): ocultarlo en el cliente habría dejado las subcategorías de Marketing viajando en cada carga.
*   **Endpoint de Alta (`src/pages/api/tickets/create.ts`):**
    *   **Autorización por Categoría:** Antes solo comprobaba que hubiera sesión. Ahora Marketing exige `tckt_mkt` + sección `crear_ticket_marketing` y el resto `tckt_csh` + `crear_ticket_csh`, o **403**. Los dos asistentes comparten endpoint, así que la categoría no basta para saber de dónde viene la petición: la restricción tiene que ser de autorización, no de origen.
*   **Middleware (`src/middleware.ts`):**
    *   **`flagRouteMap`:** Segunda pasada tras el mapa de secciones que exige el flag del usuario en las dos vistas de alta. Cierra el paso a los 90 usuarios con `tckt_mkt = false` que entraban por URL, ya que las tres roles tienen la sección `crear_ticket_marketing`. Orden resultante: sección global → override de usuario → rol → flag.
    *   **`denegarAcceso()`:** Extraído para que la denegación por flag use la misma cookie de flash y el mismo redirect que la denegación por sección.
*   **Dashboard de Marketing (`src/pages/tickets/marketing/dashboard.astro`):**
    *   **Tarjeta "Levantar ticket" Condicionada:** Era una segunda entrada visible para cualquier admin, incluido uno sin el flag. Ahora se pinta con la misma regla del Sidebar.
*   **Menú Lateral (`src/components/shared/Sidebar.astro`):**
    *   **Destino del Logo Según las Asignaciones:** El `href` fijo a `/` pasa a `logoHref`, que vale `/tickets/marketing/dashboard` cuando **todas** las asignaciones activas del usuario en `asignaciones_categorias` son de Marketing (por `categoriaId`, o por la categoría de la que cuelga su `subcategoriaId` cuando la fila no fija categoría). En producción lo cumplen dos usuarios: **Haide Herrera** (id 2) y `alumno.prueba1` (id 290).
    *   **Solo Si el Destino es Alcanzable:** la comprobación únicamente corre para quien tiene la sección `marketing_dashboard`, porque el middleware protege esa ruta; en otro caso el logo habría rebotado a `/` con el aviso de acceso denegado. Las dos consultas nuevas comparten el `Promise.all` del `findFirst` de `hasMarketingTickets`, así que el Sidebar sigue costando un viaje a RDS por render.
*   **Configuración (`src/config/ticket-categories.ts`):**
    *   **`MARKETING_CATEGORY_ID` Centralizado:** El literal `12` estaba en doce archivos, el último de ellos el Sidebar con un `categoriaId: 12` a pelo. El módulo nuevo exporta además `MARKETING_CREATE_SECTION`, `CSH_CREATE_SECTION` e `isMarketingCategory()`.
*   **Auditoría de Esquema:** Inventario de tablas y columnas sin uso tras la actualización de roles, en el `CHANGELOG.md`. Sin cambios de esquema aplicados.
