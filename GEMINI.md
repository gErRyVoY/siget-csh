...
**Tarea Actual:** Completada — Toggles Homeoffice/Vacaciones, Correcciones de UI y Pulido Final de Incidencias ✅

**Estado:** Completado. Se implementaron los toggles de "Homeoffice" (solo sábados) y "Vacaciones" (cualquier día) para incidencias con inasistencia total. Al omitirse un día (toggle "Omitir" activo), se ocultan dinámicamente los campos "Motivo (Turno)" y "Motivo (Comida)". Se corrigió la lógica de comida para que "Horario excedido" solo se muestre con registros completos de almuerzo (entrada y salida de comida). En el reporte, se actualizó la leyenda a "A tiempo" y se muestran registros individuales parciales de comida. Finalmente, se eliminó el número de semanas del asunto del correo en reportes quincenales.
